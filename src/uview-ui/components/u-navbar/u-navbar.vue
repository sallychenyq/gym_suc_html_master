	...
	<view class="u-navbar">
		<view
			...
			:style="{
				...
				height: $u.addUnit(navbarPlaceholderHeight),
			}"
		></view>
...
<script>
	...
	export default {
		...
		data() {
			return {
        navbarPlaceholderHeight:uni.$u.getPx(this.height) + uni.$u.sys().statusBarHeight
			}
		},
		methods: {
			// 点击左侧区域
    ...
    created() {
      setTimeout(()=>{
        this.navbarPlaceholderHeight=this.$u.getPx(this.height) + this.$u.sys().statusBarHeight
        console.log('重设navbarPlaceholderHeight',this.navbarPlaceholderHeight)
      },100)
    },
