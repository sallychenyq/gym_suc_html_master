<template>
	<view :style="bgColor?`background-color: ${bgColor}`:''">
		...
		<page-head ref="pageTitle" v-if="isTabBarPage && title" :show-qr-button="showQrButton"
			:show-back-btn="!isTabBarPage" @qr="$emit('qr')">
			{{ title }}
		</page-head>
		<view>
			<slot />
		...
		<u-popup closeable bgColor="transparent" :show="showPopup" @close="onClosePopupBox" @open="showPopup=true"
			mode="center" :round="10" :safeAreaInsetBottom="false">
			<view>
				<swiper autoplay :indicator-dots="true" class="swiper">
					<swiper-item v-for="(popup, index) in popupList" :key="index" class="popup-imgbox">
						<u-image @click="clickPopup(index)" class="popup-img" :radius="10" mode="widthFix" height="auto"
							:src="popup.popupImg" :fade="true" duration="450" :width="`${3.75*popup.popupSize}`">
