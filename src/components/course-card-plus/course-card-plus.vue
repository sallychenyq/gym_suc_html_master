...
<!--				<text class="text">{{ item.mustCliqueShortName }}定制</text>-->
    ...
    <view class="time-bg flex-row">
      <view :class="[item.canAppoint?'':'disable']" class="time-bg-tag flex-col">
        <text class="time">{{ item.startTimeView }}-{{ item.endTimeView }}</text>
      </view>
    </view>
		<view class="course-main-box flex-row">
			<view class="avatar-box flex-col">
				<view class="flex-col">
                    ...
                    shape="circle"
						width="132rpx" height="132rpx" @click="$v_router.push(`/pages/coach-home/index?id=${item.coachId}`)"></u-image>
				</view>
			</view>
			<view class="course-box flex-col" @click="$v_router.push(`/pages/course-detail/index?id=${item.id}`)">
				<view class="course-info1 flex-row">
					<text class="course-name">{{ item.courseName }}</text>
					<view class="record-box-title flex-col"></view>
					<text v-if="!(item.maxLimit<0)" class="course-count"><text class="appoint-count">{{ item.appointCount }}</text>/{{ item.maxLimit || '无限制' }}</text>
				</view>
				...
				<view v-if="!(item.maxLimit<0)" class="course-pay-mode">
					...
					<text class="course-pay-mode-view">{{ item.payModeFrontView }}<text v-if="item.refundTip && !item.appointed">预计可返现{{item.refundTip}}元</text></text>
				</view>
			</view>
      <view v-if="!(item.maxLimit<0)">
        <view v-if="item.appointed">
          <view v-if="item.canCancelAppoint" class="appoint-button flex-col appointed" @click="cancelAppoint">
        ...
        <view v-else>
          <button v-if="item.canAppoint" class="appoint-button flex-col" @click="appoint">
            <text class="appoint-button-view">预约</text>
          </button>
          <button v-else class="appoint-button flex-col disable">
            <text class="appoint-button-view">{{ item.cannotAppointReason }}</text>
          </button>
        </view>
      </view>
		</view>
