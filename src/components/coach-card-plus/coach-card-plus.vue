...
<!--      <text class="coach-name">{{ item.realName }}|{{ item.nickName }}</text>-->
      <view class="coach-price-box">
        <view class="coach-price" v-if="haveAppointButton && tip">
          <u-icon v-if="promotion" :color="$utils.getColors().main" name="volume" :label="tip" :labelColor="$utils.getColors().main"></u-icon>
          <text v-else>{{ tip }}</text>
        </view>
        <text class="coach-price" v-else-if="haveAppointButton">{{ item.sellPackPriceStart>0?`${item.sellPackPriceStart}元`:'' }}{{(item.sellPackPriceStart>0 && item.sellPackPriceEnd>0)?'-':''}}{{ item.sellPackPriceEnd>0?`${item.sellPackPriceEnd}元`:'' }}</text>
      </view>
      <view v-if="haveAppointButton" class="avatar-box flex-col">
        <view class="flex-col">
          <u-image :showLoading="true"
                    :src="item.avatar"
                    shape="circle"
                    width="88rpx" height="88rpx" @click="$v_router.push(`/pages/coach-home/index?id=${item.id}`)"></u-image>
        </view>
      </view>
...
<!--          <text class="all-count"><text style="color: black">{{ item.courseCount }}</text>节</text>-->
        ...
        <view class="coach-likes-box flex-row justify-between" v-if="haveAppointButton">
          <view class="coach-likes-box-main flex-row" :class="moreMode?['box-box-shadow']:[]">
            <view class="coach-likes-item flex-col justify-center" v-for="(packName,index) in getSellPackNames()" :key="index">
              <text class="coach-likes-item-text">{{ packName }}</text>
            </view>
            <view class="coach-likes-item flex-col justify-center" v-if="moreMode" v-for="(packName,index) in getSellPackNamesMore()" :key="index">
              <text class="coach-likes-item-text">{{ packName }}</text>
            </view>
          </view>
          <image
              ...
              :src="moreMode?'/static/imgs/more-course-button-close.png':'/static/imgs/more-course-button.png'"
              @click="showMore()"
          />
          <image
              v-else
      ...
      <button v-if="haveAppointButton"
              class="more-button-img flex-col"
              @click="$v_router.push(`/pages/coach-1v1/index?id=${item.id}&goodat=${coachGoodAt}${promotion?`&promotion=${promotion}`:''}`)">
        <text class="more-button-img-text">{{promotion?'试课':'预约'}}</text>
      </button>
    </view>
  </view>
</template>
