<template>
  ...
  <view class="card flex-col">
    ...
    <view
      class="course-main-box flex-row"
      @click="
        $v_router.push(`/pages/course-personal-detail/index?id=${item.id}`)
      "
    >
      <view class="avatar-box flex-col">
        <view class="flex-col">
          <u-image
            ...
            "
          ></u-image>
          <text class="course-coach-name"
            >{{ item.coachRealName
            }}{{ item.coachNickName ? `/${item.coachNickName}` : "" }}</text
          >
        </view>
      </view>
      <view class="course-box flex-col">
        ...
        <view class="course-site">
          ...
          <text class="course-site-name" v-if="item.haveNextCourse"
            >{{ $utils.timeago(item.nextCourseTime) }}
            {{ $utils.weekStr(item.nextCourseTime) }}</text
          >
        ...
        <view class="course-pay-mode">
          ...
          <text class="course-pay-mode-view"
            >{{ item.minJoinCount }}节
            <text v-if="item.signed"
              >(已签:{{ item.contract.joinCount }}节)</text
            >
          </text>
        </view>
        <block v-if="item.dutyTimesView && item.dutyTimesView.length > 0">
          ...
          <view
            class="course-pay-mode"
            v-for="(classes, index) in item.dutyTimesView"
            :key="index"
          >
