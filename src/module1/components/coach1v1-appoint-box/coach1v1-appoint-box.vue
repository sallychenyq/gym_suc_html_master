  ...
  <view>
      ...
      <view class="coach-appoint-box flex-col">
        <view class="coach-appoint-box1 flex-col">
          <view class="question-box flex-row justify-between">
            <text class="question"
              >确认预约该教练({{ coachGoodAt.typeName }})&nbsp;？</text
            >
          </view>
          <text class="class-time"
            >私教一对一&nbsp;|&nbsp;{{ appointInfo.timeStart }}-{{
              appointInfo.timeEnd
            }}&nbsp;{{ appointInfo.weekStr }}&nbsp;|&nbsp;{{
              appointInfo.timeAgo
            }}
          ...
          <coach-card-plus
            :item="coach"
            :have-appoint-button="false"
          ></coach-card-plus>
          <view class="site-pack-tips flex-row justify-between">
            <text class="site-pack-tip">请选择您要使用的私教课包:</text>
          </view>
          <button class="select-user-pack-button" @click="selectUserPackClick">
            {{ selectUserPack.packName }}
          </button>
          <view class="site-pack-tips flex-row justify-between">
          ...
          <view
            v-if="appointInfo.selectSitePack"
            class="site-pack-tips flex-row justify-between"
          >
            <text class="site-pack-tip">请选择您要使用的场地扣款方式:</text>
          </view>
          <button
            v-if="appointInfo.selectSitePack"
            class="select-user-pack-button"
            @click="selectSitePackProofClick"
          >
            {{ selectSitePackAppointProof.proofName }}
          </button>
          <view class="block3 flex-row justify-between">
            ...
            <button class="Button132 flex-col" @click="doAppoint">
              <text class="checkin-qr-box-main-text">
                {{
                  appointInfo.selectSitePack > 0 &&
                  sitePackItems[appointInfo.selectSitePack]
                    ? `确定,场地:${
                        sitePackItems[appointInfo.selectSitePack].packName
                      }`
                    : "确定,不使用场地"
                }}
              </text>
    ...
    >
      ...
      <view class="slot-content cancel-box-text">
        <u-text
          lineHeight="50rpx"
          text="取消课程后，如果您消费了课包，您的课时余额将会返还到课包账户"
        ></u-text>
      </view>
    </u-modal>
  </view>
...
import ScrollViewBox from "../../../components/scroll-view-box/scroll-view-box";
  ...
  methods: {
    selectUserPackClick() {
      ...
      for (let i = 0; i < this.userPacks.length; i++) {
        const name =
          this.userPacks[i].sellPackName +
          `(剩${this.userPacks[i].courseBalance}节)`;
        items.push({
          name: name,
          action: () => {
    ...
    selectSitePackProofClick() {
      ...
      for (let i = 0; i < this.sitePackAppointProof.length; i++) {
        const name =
          this.sitePackAppointProof[i].nameView +
          `(${this.sitePackAppointProof[i].payView})`;
        items.push({
          name: name,
          action: () => {
            this.selectSitePackAppointProof.proofName = name;
            this.selectSitePackAppointProof.proof =
              this.sitePackAppointProof[i];
          },
        });
      }
    ...
    doAppoint() {
      getApp().globalData.ignoreReClick(() => {
        ...
        if (
          this.selectPersonalType &&
          this.selectPersonalType > 0 &&
          this.appointInfo.selectSitePack <= 0
        ) {
        }
        this.$api
