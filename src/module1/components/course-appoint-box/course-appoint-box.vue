<template>
  <view>
            ...
            <view class="appoint-box flex-col">
              <view class="appoint-box-info flex-col">
                ...
                <text class="appoint-time-info">
                  {{ course.courseName }}&nbsp;|&nbsp;{{
                    course.startTimeView
                  }}-{{ course.endTimeView }}&nbsp;&nbsp;{{
                    course.startDateWeekView
                  }}
                </text>
                <view class="appoint-coach-box flex-col justify-center">
                  <view class="appoint-coach-info-box flex-row justify-between">
                    ...
                    <view class="appoint-coach-info-text flex-col">
                      <text class="appoint-coach-name"
                        >{{ course.coachRealName }}/{{
                          course.coachNickName
                        }}</text
                      >
                      <view class="appoint-site-box">
                        <text class="appoint-site-label">场地：</text>
                        <text class="appoint-site-view">{{
                          course.siteName
                        }}</text>
                      </view>
                      <view class="appoint-paymode-box">
                        <text class="appoint-paymode-label">收费：</text>
                        <text class="appoint-paymode-view"
                          >{{ course.payModeView
                          }}<text v-if="payModeIsPack(course.payMode)"
                            >({{ course.packPayCount }}课时)</text
                          ></text
                        >
                      </view>
                    </view>
                ...
                <view v-if="course.appointProofSelect">
                  ...
                  <u-list
                    :height="
                      course.appointProofSelect.length * 80 > 210
                        ? 210
                        : course.appointProofSelect.length * 80
                    "
                    showScrollbar
                  >
                    <u-list-item
                    ...
                    >
                      <view
                        :class="[
                          'sellpack-one-box',
                          'flex-col',
                          appointCourseProof === index ? 'selected' : '',
                        ]"
                        @click="
                          () => {
                      ...
                      >
                        <view class="sellpack-one flex-row justify-between">
                          <view
                            class="sellpack-one-info-box flex-col justify-between"
                          >
                            <view class="sellpack-one-title">{{
                              item.nameView
                            }}</view>
                            <view class="sellpack-one-info">
                              <text class="sellpack-course-count">{{
                                item.payView
                              }}</text>
                              <text v-if="!item.canUse" class="sellpack-ks"
                                >:{{ item.cannotUseView }}</text
                              >
                              <!--                              <text class="sellpack-price">{{ item.packAmt }}</text>-->
                              <!--                              <text class="sellpack-jg">元</text>-->
                              <text
                                v-if="item.remark"
                                class="sellpack-ont-price-info"
                                >({{ item.remark }})</text
                              >
                            </view>
                          </view>
                          <view
                            v-if="appointCourseProof !== index"
                            class="sellpack-check flex-col"
                          ></view>
                          <image
                            v-else
                ...
                <view class="appoint-buttons flex-row justify-between">
                  <button
                    class="appoint-button-close flex-col"
                    @click="closeActive"
                  >
                    <text class="appoint-button-close-view">取消</text>
                  </button>
                  <button
                    class="appoint-button-yes flex-col"
                    @click="appointCourse"
                  >
          ...
          <view class="balance-box-title flex-col">
            <view class="appoint-box flex-col">
              <view class="appoint-box-info flex-col">
                ...
                <text class="appoint-time-info">
                  {{ live.name }}&nbsp;|&nbsp;{{ live.startTimeView }}-{{
                    live.endTimeView
                  }}&nbsp;&nbsp;{{ live.startDateWeekView }}
                </text>
                <view class="appoint-coach-box flex-col justify-center">
                  <view class="appoint-coach-info-box flex-row justify-between">
                    <view class="appoint-coach-avatar flex-col">
                    ...
                    <view class="appoint-coach-info-text flex-col">
                      <text class="appoint-coach-name"
                        >{{ live.coachRealName }}/{{ live.liveAnchor }}</text
                      >
                      <view class="appoint-site-box">
    ...
    <u-popup
      :show="coursePersonalSignBox.open"
      mode="bottom"
      round="10"
      :closeable="true"
      @close="closeActive"
              ...
              <view class="appoint-box-info flex-col">
                ...
                <view class="appoint-coach-box flex-col justify-center">
                  <view class="appoint-coach-info-box flex-row justify-between">
                    ...
                    <view class="appoint-coach-info-text flex-col">
                      <text class="appoint-coach-name"
                        >{{ coursePersonal.coachRealName }}/{{
                          coursePersonal.coachNickName
                        }}</text
                      >
                      <!--                      <view class="appoint-site-box">-->
                      ...
                      <view class="appoint-paymode-box">
                        <text class="appoint-paymode-label">收费：</text>
                        <text class="appoint-paymode-view"
                          >需课包余额大于{{ coursePersonal.minJoinCount }}</text
                        >
                      </view>
                    </view>
                  </view>
                ...
                <view class="appoint-buttons flex-row justify-between">
                  ...
                  <button
                    class="appoint-button-yes flex-col"
                    @click="doSignCoursePersonal"
                  >
                    <text class="appoint-button-yes-view"
                      >签约{{ coursePersonalSignBox.joinCount }}课时</text
                    >
                  </button>
    ...
    openAppointCourse(id) {
      this.$api
        ...
        .then((res) => {
          ...
          if (
            res.data.appointProofSelect &&
            res.data.appointProofSelect.length > 0
          ) {
      ...
      this.$api
        ...
        .then((res) => {
          ...
          if (this.course.courseCate === 0) {
            message =
              "取消课程后，如果您消费了课包，您的课时余额将会返还到课包账户，确认取消预约吗?";
          } else {
            message =
              "您正在取消预约的课程，课程取消后，将不会再收到预约提醒，确认取消预约吗?";
          }
          this._confirm({
            title: "确认取消预约",
            okButton: "取消预约",
    ...
    appointCourse() {
      getApp().globalData.ignoreReClick(() => {
        ...
        try {
          if (
            this.appointCourseProof >= 0 &&
            this.course.appointProofSelect[this.appointCourseProof]
          ) {
